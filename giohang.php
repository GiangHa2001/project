...
?>
    ...
    <section class="manicontent my-3">
        ...
        <div class="product-title border-bottom">
            <strong class="bg-danger text-white fs-4 ">GIỎ HÀNG</strong>
        </div>
        </div>
        </div>
<!--phần khác-->
...
$username=$_GET['id'];
$tt=0;
$query = mysqli_query($ketnoi, "SELECT * FROM giohang left join sanpham on giohang.idsp = sanpham.idsp where username='$username' " );
...
$tt=$tt+$row['gia'];
                ...
                <td>". $row['gia'] ." </td>
                <td>
                <a href='delesp.php?id=$username&idsp=". $row['idsp'] ."&idgh=". $row['idgh'] ."'>
                <input style='float: center; border: 5px inset #d63745; background-color: #d63745; color: antiquewhite; width:40%' type='submit' value='Xóa'></a>
                </td>
            </tr>";}
                ?>
                <tr>
